...
# =========================================================================================
# LIBRERÍA CENTRAL DE SIMETRÍAS
# =========================================================================================
class SymmetryGroup:
    """Clase centralizada para manejar grupos de simetría cristalográfica y muestral."""
    def __init__(self, euler_angles, name="Unknown"):
    ...
    @staticmethod
    def cubic():
        """Genera las 24 rotaciones propias de la simetría cúbica (m-3m)."""
        matrices = []
        ...
        return SymmetryGroup(rots.as_euler('ZYZ'), "cubic")

    @staticmethod
    def triclinic():
        """Simetría triclínica (solo la identidad)."""
        rots = Rotation.from_matrix([np.eye(3)])
        return SymmetryGroup(rots.as_euler('ZYZ'), "triclinic")
...
# Diccionario global para cachear autovectores y evitar recálculos si se pasa la simetría cruda
_SYMMETRY_EIGEN_CACHE = {}

def eval_sym_sph_harm(l, polar, azimuthal, sym_or_proj, is_sample=False):
    """
    Evalúa armónicos esféricos simetrizados en grillas angulares. 
    Función "Polimórfica": acepta tanto la matriz de autovectores precalculada (mayor velocidad)
    como un objeto de simetría directo (mayor flexibilidad de uso), optimizándose con caché interna.
    """
    polar, azimuthal = np.atleast_1d(polar), np.atleast_1d(azimuthal)
    
    # 1. Lógica de Enrutamiento Inteligente
    if isinstance(sym_or_proj, np.ndarray):
        # Recibió la matriz de autovectores calculada (comportamiento legacy rápido)
        proj_matrix = sym_or_proj
    else:
        # Recibió un objeto de simetría Orix/MTEX. Calculamos los autovectores "on-the-fly".
        # Usamos caché para que no repita el cálculo de matrices pesadas si se llama dentro de un bucle G.
        sym_name = getattr(sym_or_proj, 'name', str(sym_or_proj))
        cache_key = (l, sym_name)
        
        if cache_key not in _SYMMETRY_EIGEN_CACHE:
    ...
    # 2. Evaluación Vectorizada de los armónicos base Y_l^m
    Y_raw = np.zeros((2 * l + 1, len(polar)), dtype=complex)
    for m in range(-l, l + 1):
        val = eval_sph_harm(m, l, polar, azimuthal)
        if is_sample:
            # Identidad cuántica: el conjugado de la muestra
            val = np.conj(val) * (-1.0)**m
        Y_raw[m + l, :] = val
        
    # 3. Proyección final al subespacio simétrico (k_cs o k_ss)
