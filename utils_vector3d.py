...
def proyectar_igual_area(vectores_3d, pesos):
    """
    Proyección de Igual Área (Lambert).
    Centro = Z, Superior (Norte) = Eje X, Derecha (Este) = Eje Y
    """
    ...
    # Factor de escala para Igual Área (Lambert)
    factor = np.sqrt(1 / (1 + np.abs(z_vec))) 

    # Mapeo de ejes solicitado
    x_plot = y_vec * factor
    y_plot = x_vec * factor
...
def plot_pole_figure_manual(vectores, pesos=None, titulo="Pole Figure", hkl_label=""):
    """
    Genera una Figura de Polos con convención: X arriba, Y derecha, Z centro.
    """
    ...
    ax.text(0, 1.07, 'X', fontsize=14, fontweight='bold', ha='center', va='bottom')
    ax.text(1.07, 0, 'Y', fontsize=14, fontweight='bold', ha='left', va='center')
