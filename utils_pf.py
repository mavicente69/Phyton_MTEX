...
class PoleFigure:
    ...
    def plot(self, ax=None, cmap='jet', niveles=25, max_val=None):
        """
        ...
        ax.set_title(f"Polo {{{label_plano}}}", fontsize=14)
...
def plot_pfs(lista_pfs, titulos=None, cmap='jet', max_val_global=False):
    """
    ...
    for i, ax in enumerate(axes):
        ...
        pf.plot(ax=ax, cmap=cmap, max_val=max_val)
...
def plot_pf_comparison(pfs_in, pfs_out, titulos=None, suptitle="Comparativa de Figuras de Polos", cmap='jet', unificar_escala='hkl'):
    """
    ...
    for i in range(n):
        ...
        # Fila 1: Entrada
        pf_in.plot(ax=axes[0, i], cmap=cmap, max_val=max_in)
        titulo_base = axes[0, i].get_title()
        
        # Fila 2: Salida
        pf_out.plot(ax=axes[1, i], cmap=cmap, max_val=max_out)
        
        # Aplicamos títulos personalizados o usamos los por defecto
        if titulos is not None:
            titulo_base = titulos[i]
            
        axes[0, i].set_title(f"Entrada - {titulo_base}", fontsize=14)
        axes[1, i].set_title(f"Salida - {titulo_base}", fontsize=14)
