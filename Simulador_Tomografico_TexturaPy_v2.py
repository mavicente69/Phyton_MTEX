...
CORREGIDO: El tilteo aplica la rotación 3D alrededor del centro de masa.
ULTRA-OPTIMIZADO (GENERAL): Proyección vectorial de coeficientes de Fourier.
Se apilan los mapas 3D de todos los armónicos en el eje Z para que TomoPy 
calcule todo en una única integral C++, respetando la textura vóxel a vóxel.
CORREGIDO MKL: Se limitó tomopy a ncore=1 para evitar el colapso de la memoria de paginación en Windows.
"""
...
def liberar_recursos_memoria():
    """Fuerza el cierre de figuras y limpia la RAM para evitar Errno 22 en Windows."""
    plt.close('all')
    gc.collect()
...
def crear_volumen_multimaterial(N, radio_cilindro, altura_cilindro):
    ...
    0 = Vacío/Aire | 1 = Material 1 | 2 = Material 2
    """
    ...
    radio_nucleo = radio_cilindro * 0.5 
...
def proyectar_coeficientes_textura(mascara_mat, C_bunge, angulos_rad, voxel_size_cm):
    """
    OPTIMIZACIÓN VECTORIAL ESTRICTA: 
    Genera un volumen 3D para cada coeficiente activo (soportando heterogeneidad 
    vóxel a vóxel) y los apila a lo largo del eje Z. Se envía el 'mega-volumen' a
    tomopy.project de una sola vez y se desempaqueta el resultado.
    """
    proyecciones_C = {}
    keys_list = []
    vols_list = []
    
    # 1. Componente Isótropa (Background / Densidad)
    keys_list.append('background')
    vols_list.append((mascara_mat * voxel_size_cm).astype(np.float32))
    
    # 2. Componentes Anisótropas (Armónicos Activos)
    ...
    for (l, mu, nu), c_val in coefs_activos.items():
        ...
        # c_array podría ser en el futuro una matriz 3D completa (textura variable)
        # Al multiplicarlo por mascara_mat, se garantiza que es un volumen 3D.
        if np.max(np.abs(c_array.real)) > 1e-10:
            keys_list.append((l, mu, nu, 'real'))
            vols_list.append((mascara_mat * c_array.real * voxel_size_cm).astype(np.float32))
            
        if np.max(np.abs(c_array.imag)) > 1e-10:
            keys_list.append((l, mu, nu, 'imag'))
            vols_list.append((mascara_mat * c_array.imag * voxel_size_cm).astype(np.float32))
            
    # Para evitar bloqueos de RAM si L_max es masivo, iteramos en lotes manejables.
    batch_size = 20  # REDUCIDO a 20 para no saturar el archivo de paginación de Windows
    Nz = mascara_mat.shape[0]  # Profundidad original en Z
    
    print(f"    [OPTIMIZACIÓN] Vectorizando {len(vols_list)} tensores 3D en lotes de {batch_size}...")
    t0 = time.time()
    
    for i in range(0, len(vols_list), batch_size):
        batch_vols = vols_list[i:i+batch_size]
        batch_keys = keys_list[i:i+batch_size]
        
        # Apilamos las matrices 3D a lo largo del eje Z.
        stacked_vol = np.concatenate(batch_vols, axis=0)
        
        # Proyección vectorial de un solo golpe
        # IMPORTANTE: ncore=1 previene que TomoPy clone la memoria en múltiples hilos y colapse MKL
        stacked_proj = tomopy.project(stacked_vol, angulos_rad, ncore=1)
        
        # Desempaquetado del resultado
        for j, key in enumerate(batch_keys):
            proyecciones_C[key] = stacked_proj[:, j*Nz:(j+1)*Nz, :]
            
    print(f"    [OK] Proyecciones vectoriales completadas en {time.time()-t0:.2f}s.")
...
def ensamblar_radiografias_multimaterial(lambda_test, ws_data, tilt_deg):
    ...
    for id_mat, info_mat in ws_data['materiales'].items():
        ...
        atenuacion_total += atenuacion_mat 
...
if __name__ == '__main__':
    # =====================================================================
    # PANEL DE CONTROL (MAIN)
    # =====================================================================
    CONFIGURACION_MAIN = {
        # --- NUEVO: DIRECTORIO BASE DE SALIDA ---
        # IMPORTANTE: Usa siempre una 'r' antes de las comillas si pones rutas de Windows (ej: r'D:\Mi_Carpeta')
        'directorio_salida': r'Resultados_Tomografia', 
        
        'LAMBDA_MIN': 1.0,
        'LAMBDA_MAX': 6.0,
        'NUM_LAMBDA_PUNTOS': 300,   
        ...
        'L_max': 0,  # L_max altos soportados sin colapsar en tiempo
        'N_voxels': 70,           
        'radio_cilindro': 25,     
        'altura_cilindro': 40,    
        ...
        'lambdas_espectro': np.linspace(1.0, 6.0, 300), 
        'lam_detector': 2.5,
        'angulos_detector': [0.0, 45.0, 90.0],
        
        'lambdas_export_csv': np.linspace(1.0, 6.0, 300),
        'lambdas_export_volumen': np.linspace(1.0, 6.0, 300), 
        'exportar_csv': True,
        'exportar_npy': True,
...
# %% [BLOQUE 3] INTERACTIVO: COMPARAR DOS IMÁGENES
if __name__ == '__main__':
    try:
...
# %% [BLOQUE 4] INTERACTIVO: PERFILES DEL DETECTOR 1D
if __name__ == '__main__':
    try:
...
# %% [BLOQUE 5] INTERACTIVO: ESPECTROS MÚLTIPLES PÍXELES
if __name__ == '__main__':
    try:
...
# %% [BLOQUE 8] EXPORTACIÓN DE DATOS 4D
if __name__ == '__main__':
    try:
        out_dir_base = ws['CONFIG'].get('directorio_salida', "Resultados_Tomografia")
...
# %% [BLOQUE 9] EXTRACCIÓN DE PROYECCIONES DIRECTAS DE A_lmu
if __name__ == '__main__':
    try:
        if ws['CONFIG'].get('exportar_proyecciones_almu', True):
            out_dir_base = ws['CONFIG'].get('directorio_salida', "Resultados_Tomografia")
            out_dir_almu_base = os.path.join(out_dir_base, "Proyecciones_Almu_Detector")
